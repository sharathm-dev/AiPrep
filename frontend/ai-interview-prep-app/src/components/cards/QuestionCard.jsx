...
import { LuChevronDown, LuChevronUp, LuPin, LuPinOff, LuLightbulb, LuSparkles } from 'react-icons/lu';
...
const QuestionCard = ({
    ...
    useEffect(() => {
        if (isExpanded) {
            const contenHeight = contenRef.current.scrollHeight;
            setHeight(contenHeight + 10);
        } else {
            setHeight(0);
        }
    }, [isExpanded]);
        ...
        <div className="bg-white rounded-lg mb-4 overflow-hidden py-4 px-5 shadow-xl shadow-gray-100/70 border border-gray-100/60 group">
            <div className="flex items-start justify-between cursor-pointer ">
                <div className="flex items-start gap-3.5">
                    <span className="text-xs md:text-[15px] font-bold text-gray-500 leading-[18px]">
                        Q
                    </span>
                    <h3
                        className="text-xs md:text-[14px] font-semibold text-gray-900 mr-0 md:mr-20"
                ...
                <div className="flex items-center justify-end ml-4 relative">
                    ...
                    >
                        <button
                            className="flex items-center gap-2 text-xs text-indigo-800 font-medium bg-indigo-50 px-3 py-1 mr-2 rounded text-nowrap border-indigo-50 hover:border-indigo-200 cursor-pointer"
                            onClick={onTogglePin}
                        >
                            {isPinned ? (
